...
plt.plot(df_P_c['t'], df_P_c['val'] / 133.322, label='Modèle Windkessel couplé (ODE)', color='blue', linewidth=2)


plt.ylabel('Pression (mmHg)')
plt.title('Pression et débit dans l artère avec modèle Windkessel couplé (ODE)')
plt.legend()
...
plt.plot(df_Q_c['t'], df_Q_c['val'] * 1e6, label='Débit Windkessel couplé (ODE)', color='blue', linewidth=2)
